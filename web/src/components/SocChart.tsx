...
// Тёмные полосы — тень, жёлтые засечки — калибровки.
export default function SocChart({ cells, dark, steps, reserve = 30, critical = 20 }: {
  cells: Cell[]; dark: string; steps: number; reserve?: number; critical?: number;
}) {
  const W = 1000, H = 180, P = 28;
  const x = (k: number) => P + (k / steps) * (W - P - 8);
  const y = (v: number) => 8 + (1 - v / 100) * (H - P);
  const line = cells.map((c, i) => `${i ? "L" : "M"}${x(c.step + 1).toFixed(1)},${y(c.soc).toFixed(1)}`).join("");
  const shadow = [...dark].map((d, k) => (d === "1" ? k : -1)).filter((k) => k >= 0);
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }} role="img"
      aria-label="График заряда аппарата">
      {shadow.map((k) => <rect key={k} x={x(k)} y={8} width={x(1) - x(0) + 0.5} height={H - P} fill="#0b0e14" />)}
      {[0, 50, 100].map((v) => (
        <g key={v}>
      ...
      {cells.filter((c) => c.executed === "calibrate").map((c) => (
        <line key={c.step} x1={x(c.step)} x2={x(c.step)} y1={H - P} y2={H - P + 6} stroke="#ffca16" />
      ))}
