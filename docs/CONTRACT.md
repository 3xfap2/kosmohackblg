...
server/       FastAPI без состояния: HTTP ↔ core (Vercel Function), маршруты /api/…
...
`server/` вызывает только эти функции. Все чистые: запись на входе не меняется, возвращается новая.
...
| `/api/runs/verify` | `{run}` | выгрузка и повтор официальной моделью на сервере: `{match, summary, diff?}` |
